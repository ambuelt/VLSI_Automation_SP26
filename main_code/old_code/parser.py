...
def connect_outputs(output_wires, nodes: dict):
    ...
    for output in output_wires:
        ...
        # Checks through all the node value to see which Node has same ending as a primary output
        for node in nodes.values():

            # DO THIS IN ORDER TO NOT REPEAT OUTPUTS TWICE!!! <- If not included will create another output node for indgrees in topology sort
...
def parse_bench(file, lut=None):
    ...
    if lut is not None:
        ...
        forward_sta(nodes, lut)     # Compute the forward STA to be able to calculate forward circuit delay
        ...
        backward_sta(nodes, delay)  # Compute the backward STA to be able to calculate forward slack and critcal path
